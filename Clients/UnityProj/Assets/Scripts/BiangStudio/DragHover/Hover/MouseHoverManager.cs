        ...
        public class PressHoverImmediately<T> : HoverActionBase where T : MonoBehaviour
        {
            ...
            public override void Check()
            {
                if (Instance.KeyDownHandler())
                {
                    ...
                    RaycastHit raycast;
                    Physics.Raycast(ray, out raycast, 100f, LayerMask);
                    Debug.DrawLine(ray.origin, ray.origin + 100f * ray.direction.normalized, Color.yellow);
                ...
                if (!Instance.KeyDownHandler())
                {
                    ...
                    else
                    {
                        ...
                        if (mouseStopTimeTicker > DelaySeconds)
                        {
                            ...
                            RaycastHit raycast;
                            Physics.Raycast(ray, out raycast, 100f, LayerMask);
                            Debug.DrawLine(ray.origin, ray.origin + 100f * ray.direction.normalized, Color.red, 1f);
                            if (raycast.collider != null)
                            {
                                MouseHoverComponent mouseHoverComponent = raycast.collider.gameObject.GetComponentInParent<MouseHoverComponent>();
