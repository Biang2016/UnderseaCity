...
namespace BiangStudio.ShapedInventory
{
    ...
    public abstract class Inventory
    {
        ...
        private void AddItem(InventoryItem item)
        {
            ...
            if (EnableLog) LogError("AddItem: " + sb.ToString());
            sb.Clear();
            RefreshConflictAndIsolation();
        }
        ...
        {
            if (InventoryInfo.PreviewItem != null)
            {
                InventoryInfo.PreviewItem = item;
                if (InventoryInfo.InventoryItems.Contains(item))
                {
                    StringBuilder sb = new StringBuilder();
                    foreach (GridPos gp_matrix in item.OccupiedGridPositions_Matrix)
                    ...
                    if (EnableLog) LogError("PickUpItem: " + sb.ToString());
                }
            }
            else
            {
                if (EnableLog) LogError("PickUpItem failed, already has a PreviewItem");
            }
        }
        ...
        {
            StringBuilder sb = new StringBuilder();
            foreach (GridPos gp_matrix in InventoryInfo.PreviewItem.OccupiedGridPositions_Matrix)
            {
                sb.Append(gp_matrix.ToString());
                InventoryGridMatrix[gp_matrix.x, gp_matrix.z].State = InventoryGrid.States.Unavailable;
            }

            if (EnableLog) LogError("PutDownItem: " + sb.ToString());
            AddItem(InventoryInfo.PreviewItem);
            InventoryInfo.PreviewItem = null;
        }
        ...
        public void RefreshConflictAndIsolation(out List<InventoryItem> conflictItems, out List<InventoryItem> isolatedItems)
        {
            ...
            if (InventoryInfo.PreviewItem != null)
            {
                InventoryInfo.PreviewItem.OnIsolatedHandler?.Invoke(false);
                InventoryInfo.PreviewItem.OnResetConflictHandler?.Invoke();
            }
    ...
    public class InventoryInfo
    {
        ...
        public Dictionary<uint, InventoryItem> InventoryItemDict = new Dictionary<uint, InventoryItem>();

        public bool AddNormalInventoryItem(InventoryItem item)
        {
        ...
        public bool RemoveNormalInventoryItem(InventoryItem item)
        {

        }

        public bool SetInventoryItemAsPreview(InventoryItem previewItem)
        {
            if (InventoryItemDict.ContainsKey(previewItem.GUID))
            {
                if (PreviewItem != null)
                {
                    if(Inventory.EnableLog) LogError("");
                    return false;
                }
                else
                {
